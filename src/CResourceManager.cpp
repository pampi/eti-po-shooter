...
void CResourceManager::loadLevel(int lvl)
{
    ...
    try
    {
        ...
        BOOST_FOREACH( boost::property_tree::ptree::value_type &root, drzewko.get_child("gui") )
        {
			...
			else if(root.first == "gui_static_overlay")
			{
				BOOST_FOREACH( boost::property_tree::ptree::value_type &gui_static, root.second)
				{
					if(gui_static.first == "gui_static")
					{
						...
						//Position
						sf::Vector2f pos;
						std::istringstream iss_position(gui_static.second.get<std::string>("<xmlattr>.position","0.0 0.0"));
						iss_position >> pos.x >> pos.y;
						...
						//height
						int height = gui_static.second.get<int>("<xmlattr>.height", 0);
			...
			else if(root.first == "gui_bars")
			{
				BOOST_FOREACH( boost::property_tree::ptree::value_type &gui_bar, root.second)
				{
					if(gui_bar.first == "bar")
					{
						// id
						std::string id_gui = gui_bar.second.get<std::string>("<xmlattr>.id","");

						// img path
						std::string iPath = gui_bar.second.get<std::string>("<xmlattr>.file_path", "");
						this->loadImageKey(iPath);

						// Position
						sf::Vector2f pos;
						std::istringstream iss_position(gui_bar.second.get<std::string>("<xmlattr>.position","0.0 0.0"));
						iss_position >> pos.x >> pos.y;

						// hidden?
						int hide = gui_bar.second.get<int>("<xmlattr>.hidden", 0);

						// width
						int width = gui_bar.second.get<int>("<xmlattr>.width", 0);

						// height
						int height = gui_bar.second.get<int>("<xmlattr>.height", 0);

						// startValue
						float startValue = gui_bar.second.get<float>("<xmlattr>.startValue", 0);

						
						// Color
						sf::Color normal_color;
						unsigned int tab_color[4];
						std::istringstream iss_color(gui_bar.second.get<std::string>("<xmlattr>.color","255 0 0 255"));
						iss_color >> tab_color[0] >> tab_color[1] >> tab_color[2] >> tab_color[3];
						...
						this->m_guiElements.push_back((CGuiElement*)(new CProgressBar(pos, id_gui, height, width, startValue, (bool)(hide!=0), iPath, normal_color)));
